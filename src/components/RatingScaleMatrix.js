...
const RatingScaleMatrix = ({ onDelete, componentId }) => {
  ...
  const scaleType = useSelector((state) => {
    const instance = state.RatingMatrix.byId[componentId];
    if (!instance) {
      return 2;
    }
    return instance.scaleType;
  });
                  ...
                  <thead>
                    {displayMode === "radio" && (
                      <tr class="column-row">
                        ...
                        {[...Array(Number(scaleType))].map((e, i) => (
                          <th style={{ textAlign: "center" }} key={i}>
                            {i + 1}
                          </th>
                        ))}
                        {/* <th class="column-label" style={{ width: "20%" }}>
                  ...
                  <tbody className="mt-[10px]">
                    {fieldsData.map((e, i) => (
                      <tr className="mt-[10px]">
                        ...
                        {displayMode === "drop" && (
                          <td className="text-center mt-[10px]">
                            <select className="border-[1px] border-solid w-[90%]">
                              {[...Array(Number(scaleType))].map((e, i) => (
                                <option>{i + 1}</option>
                              ))}
                            </select>
                        ...
                        {/* <td class="field-cell" style={{ width: "20%" }}>
                          <label class="inner">
                            <input
                              type="radio"
                              name="question[858394][1315164][123908]"
                              value="655"
                            />
                          </label>
                        </td>{" "}
